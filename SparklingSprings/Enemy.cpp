...
Enemy::Enemy(const string& _name, const ShapeData& _data)
...
void Enemy::InitHUD()
{
	Canvas* _canvas = new Canvas(STRING_ID("Enemy"), FloatRect(0, 0, 1, 1));
	float _sizeX = 200.0f; float _sizeY = 150.0f;
	ProgressBar* _healthBar = new ProgressBar(ShapeData(Vector2f(0.0f, 0.0f), Vector2f(_sizeX, _sizeY), PATH_HEALTH_BAR_EMPTY),
